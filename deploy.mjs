...
for (const vault of vaults) {
	...
	const dest = join(vault, ".obsidian", "plugins", "powerassistant");
	...
	// one-time rename migration: carry settings (API keys, folders, tokens)
	// over from the old Power Capture install when the new folder has none
	const oldData = join(vault, ".obsidian", "plugins", "powercapture", "data.json");
	const newData = join(dest, "data.json");
	if (!existsSync(newData) && existsSync(oldData)) {
		copyFileSync(oldData, newData);
		console.log("migrated settings from powercapture ->", newData);
	}
	console.log("deployed ->", dest);
	deployed++;
}
console.log(deployed ? `Done. Reload Obsidian (Ctrl+R) and enable "Power Assistant" if it isn't enabled yet.` : "Nothing deployed.");
